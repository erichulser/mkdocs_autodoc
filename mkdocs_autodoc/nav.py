...
class ModulePage(ApiPage):
    def collect_members(self):
        ...
        # add any submodules that have not yet been loaded
        if '__init__' in self.py_object.__file__:
            base_path = os.path.dirname(self.py_object.__file__)
            submodules = os.listdir(base_path)
            for submodule in submodules:
                if submodule.endswith('.py') and submodule != '__init__.py':
                    module_name = submodule.split('.')[0]
                elif os.path.exists(os.path.join(base_path, submodule, '__init__.py')):
                ...
                pkg_name = self.py_object.__name__ + '.' + module_name
                try:
                    __import__(pkg_name)
                    pkg = sys.modules[pkg_name]
                except StandardError as err:
                    print 'Failed to import: ', pkg_name, err
                else:
                    members[module_name] = pkg
